    ...
    <UserLayout title="My Assets Portfolio">
        <template #header>
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                <div>
                    <h1 class="text-xl sm:text-2xl font-black text-slate-900">My Assets Portfolio</h1>
                    <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Track your assets in MMK, USD, and SGD with
                        real-time exchange rates</p>
                </div>
                <div class="flex gap-2 sm:gap-3">
                    <button @click="toggleView"
                        class="px-3 sm:px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-xl text-xs sm:text-sm font-bold transition flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-6">
            ...
            <div
                class="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-5 border border-slate-200 shadow-sm hover:shadow-md transition-all">
                <p class="text-[10px] sm:text-xs text-slate-400 mb-2">💰 Total Invested</p>
                <div class="space-y-1.5">
                    ...
                    <div class="border-t border-slate-200 pt-2 mt-1">
                        <div class="flex justify-between items-center text-[10px] sm:text-[11px]">
                            <span class="font-bold">Total (MMK):</span>
                            <span class="font-mono font-bold">{{ formatMoney(totalInvestedByCurrency.totalMmk) }}
                                MMK</span>
                        </div>
                        <div class="flex justify-between text-[8px] sm:text-[9px] text-slate-400 mt-0.5">
            ...
            <div class="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-5 border border-slate-200 shadow-sm">
                <p class="text-[10px] sm:text-xs text-slate-400 mb-2">Currency Distribution</p>
                <div class="space-y-2">
                    <div class="flex justify-between text-[10px] sm:text-xs">
                        <span class="font-medium">🇲🇲 MMK</span>
                        <span class="font-mono">{{ currencyDistribution.mmk }}%</span>
                    </div>
                    ...
                    <div class="flex justify-between text-[10px] sm:text-xs">
                        <span class="font-medium">🇸🇬 SGD</span>
                        <span class="font-mono">{{ currencyDistribution.sgd }}%</span>
                    </div>
                    <div class="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    ...
                    <div class="flex justify-between text-[10px] sm:text-xs">
                        <span class="font-medium">🇺🇸 USD</span>
                        <span class="font-mono">{{ currencyDistribution.usd }}%</span>
                    </div>
                    <div class="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row gap-3 mb-4 sm:mb-6">
            <div class="flex-1">
                <div class="relative">
                    ...
                    <input type="text" v-model="searchQuery" placeholder="Search assets..."
                        class="w-full pl-9 pr-4 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500">
                </div>
            </div>
            <select v-model="filterType"
                class="px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white">
            ...
            <button @click="sortOrder = sortOrder === 'asc' ? 'desc' : 'asc'"
                class="px-3 py-2 border border-slate-200 rounded-xl text-sm hover:bg-slate-50 transition bg-white">
        ...
        <!-- Results Count -->
        <div v-if="assets.length > 0" class="mb-3 text-xs text-slate-400">
            Showing {{ filteredAndSortedAssets.length }} of {{ assets.length }} assets
        </div>

        <!-- Table View -->
        <div v-if="viewMode === 'table' && filteredAndSortedAssets.length > 0"
            class="bg-white rounded-xl sm:rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
            <div class="overflow-x-auto">
                <table class="w-full min-w-[1200px]">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr>
                            <th class="px-4 sm:px-6 py-3 text-left text-[10px] sm:text-xs font-bold text-slate-500">
                                Asset</th>
                            <th class="px-4 sm:px-6 py-3 text-right text-[10px] sm:text-xs font-bold text-slate-500">
                                Purchase Currency</th>
                            <th class="px-4 sm:px-6 py-3 text-right text-[10px] sm:text-xs font-bold text-slate-500">
                                Purchase Value</th>
                            <th class="px-4 sm:px-6 py-3 text-right text-[10px] sm:text-xs font-bold text-slate-500">
                                Current Value</th>
                            <th class="px-4 sm:px-6 py-3 text-right text-[10px] sm:text-xs font-bold text-slate-500">P&L
                                (Original)</th>
                            <th class="px-4 sm:px-6 py-3 text-right text-[10px] sm:text-xs font-bold text-slate-500">P&L
                                (MMK)</th>
                            <th class="px-4 sm:px-6 py-3 text-right text-[10px] sm:text-xs font-bold text-slate-500">P&L
                            ...
                            <th class="px-4 sm:px-6 py-3 text-right text-[10px] sm:text-xs font-bold text-slate-500">
                                Return %</th>
                            <th class="px-4 sm:px-6 py-3 text-center text-[10px] sm:text-xs font-bold text-slate-500">
                                Action</th>
                        </tr>
                    </thead>
                        ...
                        <tr v-for="asset in filteredAndSortedAssets" :key="asset.id"
                            ...
                            <td class="px-4 sm:px-6 py-3">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-10 h-10 rounded-xl bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center text-lg">
                                        {{ getAssetIcon(asset.type) }}
                                    </div>
                                    <div>
                                        <p class="font-bold text-slate-900 text-sm">{{ asset.name }}</p>
                                        <p class="text-[10px] text-slate-400">{{ getAssetTypeLabel(asset.type) }} · {{
                                            asset.quantity }} units</p>
                                        <p class="text-[9px] text-slate-400">{{ formatDate(asset.purchase_date) }}</p>
                                        <p v-if="asset.type === 'currency'" class="text-[8px] text-blue-600 mt-0.5">
                                            Rate: {{ formatMoney(asset.purchase_price) }} MMK/{{ asset.purchase_currency
                                            }}
                                        </p>
                                        <p v-if="asset.type === 'gold' && asset.troy_ounces"
                                            class="text-[8px] text-amber-600 mt-0.5">
                            ...
                            <td class="px-4 sm:px-6 py-3 text-right">
                                <!-- Purchase Value - Show in original currency for gold, in MMK for currency -->
                                <p class="font-mono font-bold text-sm"
                                    :class="asset.type === 'currency' ? 'text-emerald-600' : ''">
                            ...
                            <td class="px-4 sm:px-6 py-3 text-right">
                                <!-- Current Value - Show in original currency for gold, in MMK for currency -->
                                <p class="font-mono font-bold text-sm"
                                    :class="asset.type === 'currency' ? 'text-emerald-600' : 'text-blue-600'">
                                    <span v-if="asset.type === 'currency'">
                            ...
                            <td class="px-4 sm:px-6 py-3 text-right">
                                <p :class="asset.pnl_original >= 0 ? 'text-emerald-600' : 'text-rose-600'"
                            ...
                            <td class="px-4 sm:px-6 py-3 text-right">
                                <p :class="asset.pnl_mmk >= 0 ? 'text-emerald-600' : 'text-rose-600'"
                                    class="font-mono font-bold text-sm">
                                    {{ asset.pnl_mmk >= 0 ? '+' : '' }}{{ formatMoney(asset.pnl_mmk) }}
                                </p>
                            </td>
                            <td class="px-4 sm:px-6 py-3 text-right">
                                <p :class="asset.pnl_usd >= 0 ? 'text-emerald-600' : 'text-rose-600'"
                                    class="font-mono font-bold text-sm">
                                    {{ asset.pnl_usd >= 0 ? '+' : '' }}${{ formatMoney(Math.abs(asset.pnl_usd)) }}
                                </p>
                            </td>
                            <td class="px-4 sm:px-6 py-3 text-right">
                                <p :class="asset.pnl_sgd >= 0 ? 'text-emerald-600' : 'text-rose-600'"
                                    class="font-mono font-bold text-sm">
                            ...
                            <td class="px-4 sm:px-6 py-3 text-right">
                                <span
                                    :class="asset.is_profit ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'"
        ...
        <div v-else-if="viewMode === 'card' && filteredAndSortedAssets.length > 0"
            ...
            <div v-for="asset in filteredAndSortedAssets" :key="asset.id"
                class="bg-white rounded-xl sm:rounded-2xl border border-slate-200 overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                <div class="p-4 sm:p-5">
                    <div class="flex justify-between items-start mb-3 sm:mb-4">
                        <div class="flex items-center gap-2 sm:gap-3">
                            <div
                                class="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center text-xl sm:text-2xl">
                                {{ getAssetIcon(asset.type) }}
                            </div>
                            <div>
                                <h3 class="font-bold text-slate-900 text-sm sm:text-base">{{ asset.name }}</h3>
                                <p class="text-[8px] sm:text-[10px] text-slate-400">{{ getAssetTypeLabel(asset.type) }}
                                </p>
                        ...
                        <span :class="asset.is_profit ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'"
                    ...
                    <div class="space-y-2 sm:space-y-3 text-xs sm:text-sm">
                        ...
                        <div class="flex justify-between">
                            <span class="text-slate-500">P&L (MMK):</span>
                            <span :class="asset.is_profit ? 'text-emerald-600' : 'text-rose-600'"
                                class="font-mono font-bold">
                                {{ asset.is_profit ? '+' : '' }}{{ formatMoney(asset.pnl_mmk) }} MMK
                            </span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-slate-500">P&L (USD):</span>
                            <span :class="asset.is_profit ? 'text-emerald-600' : 'text-rose-600'"
                                class="font-mono font-bold">
                        ...
                        <div class="flex justify-between">
                            <span class="text-slate-500">P&L (SGD):</span>
                            <span :class="asset.is_profit ? 'text-emerald-600' : 'text-rose-600'"
                                class="font-mono font-bold">
                        ...
                        <button @click="showCalculation(asset)"
                            class="w-full text-center text-[8px] sm:text-[10px] text-blue-500 hover:text-blue-700 underline">
                            📊 View calculation details
                        </button>
                        <div class="pt-2 border-t border-slate-100">
                            <div class="flex justify-between text-[8px] sm:text-[10px] text-slate-400">
                    ...
                    <button @click="deleteAsset(asset.id)"
                        class="mt-3 sm:mt-4 w-full py-1.5 sm:py-2 text-red-500 hover:text-red-700 text-[10px] sm:text-xs font-bold border border-red-200 rounded-lg hover:bg-red-50 transition">
                        Remove Asset
                    </button>
                </div>
            </div>
        ...
        <div v-else-if="filteredAndSortedAssets.length === 0 && assets.length > 0"
            class="text-center py-12 sm:py-16 bg-white rounded-xl sm:rounded-2xl border border-slate-200">
            <div
                class="w-16 h-16 sm:w-20 sm:h-20 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
            ...
            <p class="text-slate-500 font-medium text-sm sm:text-base">No matching assets found</p>
            <p class="text-xs sm:text-sm text-slate-400 mt-1">Try adjusting your search or filters</p>
            <button @click="clearFilters"
        ...
        <div v-else-if="assets.length === 0"
            ...
            <p class="text-slate-500 font-medium text-sm sm:text-base">No assets yet</p>
            <p class="text-xs sm:text-sm text-slate-400 mt-1">Start tracking your investments by adding your first asset
            </p>
            <Link :href="route('user.assets.create')"
                class="inline-block mt-3 sm:mt-4 px-4 sm:px-6 py-1.5 sm:py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-xs sm:text-sm font-bold transition">
        ...
        <div class="mt-4 sm:mt-6 p-3 sm:p-4 bg-gradient-to-r from-slate-50 to-white rounded-xl border border-slate-200">
            <div class="flex flex-wrap justify-between items-center gap-2 sm:gap-3">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-[8px] sm:text-[10px] text-slate-500">Live exchange rates from market data</span>
                </div>
                <div class="flex flex-wrap gap-2 sm:gap-4 text-[7px] sm:text-[9px] font-mono text-slate-500">
        ...
        <div v-if="selectedAsset" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4"
            ...
            <div
                class="bg-white rounded-xl sm:rounded-2xl max-w-2xl w-full max-h-[85vh] sm:max-h-[80vh] overflow-y-auto">
                <div
                    class="sticky top-0 bg-white border-b border-slate-200 p-4 sm:p-5 flex justify-between items-center">
                    <div>
                        <h3 class="text-base sm:text-lg font-black text-slate-900">Calculation Details</h3>
                        <p class="text-xs sm:text-sm text-slate-500">{{ selectedAsset.name }}</p>
                    </div>
                ...
                <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
                    ...
                    <div class="bg-slate-50 rounded-xl p-3 sm:p-4">
                        <h4 class="text-xs sm:text-sm font-black text-slate-900 mb-2 sm:mb-3 flex items-center gap-2">
                        ...
                        <div class="space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="font-mono font-bold">{{ selectedAsset.quantity }} units</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-600">Purchase Date:</span>
                                <span class="font-mono">{{ selectedAsset.purchase_date }}</span>
                            </div>
                            <div class="border-t border-slate-200 my-2"></div>
                            <div class="flex justify-between font-bold">
                                <span>Total Purchase Value (MMK):</span>
                                <span class="font-mono text-blue-600">{{ formatMoney(selectedAsset.purchase_value_mmk)
                                    }}
                                    MMK</span>
                            </div>
                        </div>
                    ...
                    <!-- P&L Summary -->
                    <div
                        class="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-3 sm:p-4 border border-indigo-200">
                        <h4 class="text-xs sm:text-sm font-black text-indigo-800 mb-2 sm:mb-3 flex items-center gap-2">
                            <span class="w-1 h-3 sm:h-4 bg-indigo-500 rounded-full"></span>
                            📈 Profit & Loss Summary
                        </h4>
                        <div class="space-y-2 text-xs sm:text-sm">
                            <div class="flex justify-between">
                                <span class="text-indigo-700">Return %:</span>
                            ...
                            <div class="flex justify-between">
                                <span class="text-indigo-700">P&L (MMK):</span>
                            ...
                            <div class="flex justify-between">
                                <span class="text-indigo-700">P&L (USD):</span>
                            ...
                            <div class="flex justify-between">
                                <span class="text-indigo-700">P&L (SGD):</span>
                    ...
                    <div class="bg-slate-100 rounded-xl p-3">
                        <h4 class="text-[9px] sm:text-[10px] font-black text-slate-500 mb-2">📊 Exchange Rates Used</h4>
                        <div class="grid grid-cols-2 gap-1 sm:gap-2 text-[8px] sm:text-[9px]">
                            <div class="flex justify-between">
                                <span>USD/MMK:</span>
                                <span class="font-mono">{{ formatMoney(currentRates?.usd_mmk) }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span>SGD/MMK:</span>
                                <span class="font-mono">{{ formatMoney(currentRates?.sgd_mmk) }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span>USD/SGD:</span>
                                <span class="font-mono">{{ formatNumber(currentRates?.usd_sgd, 4) }}</span>
                            </div>
